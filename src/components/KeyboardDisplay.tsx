...
export function KeyboardDisplay({
  ...
  const press = (e: React.PointerEvent, key: string) => {
    e.preventDefault();
    const n = NOTE_KEYS[key];
    if (n) playNote(key, n.freq);
  };
          ...
          return (
            ...
            >
              <rect
                ...
                stroke={on ? accent : "rgba(0,0,0,0.25)"}
                strokeWidth={0.5}
              />
              {labelMode === "note" ? (
                <text
          ...
          return (
            ...
            >
              <rect
                ...
                stroke={on ? accent : "rgba(255,255,255,0.1)"}
                strokeWidth={0.5}
              />
              <text
                x={x + BW / 2}
                y={BH - 5}
                textAnchor="middle"
                fontSize={labelMode === "note" ? 5 : 6}
                fontFamily="monospace"
                fill={on ? "#080808" : "rgba(255,255,255,0.5)"}
              >
                {labelMode === "note" ? note : key.toUpperCase()}
              </text>
            </g>
          );
        })}
